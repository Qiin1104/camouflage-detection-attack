...
parser.add_argument('--pth_path', type=str, default='./pre_model/CAMO512/Net_epoch_best.pth')
opt = parser.parse_args()
writer = SummaryWriter('./snapshot/SINet_V2/xr')  # hjc 可视化工具对应文件夹
for _data_name in ['dataset_3k']:  # , 'COD10K', 'CHAMELEON'
    data_path = './{}/F_ADV/xr'.format(_data_name)
    save_path = './res/xr/{}/'.format(opt.pth_path.split('/')[-3])
    # preimg_path = './res/base/{}/{}/'.format(opt.pth_path.split('/')[-3], 'bz')
    ...
    image_root = '{}/IFGSM/'.format(data_path) #YZ/Model/Network_RS5/MIX
    ...
    writer.add_scalars('Loss-epoch', {'test': loss_all_test}, global_step=NUMBER)
